    ...
    def run(self, startDateTime, endDateTime,
        ...
        for timestamp in DF.index[3:]:
            ...
            for index, row in self.openPnl.iterrows():
                try:
                    data = backTestTools.getHistData1Min(timestamp=timestamp-60,
                                                    symbol=row['Symbol'],
                                                    conn=self.conn)
            ...
            logging.info(f'-----time=>--Time {humanTime}---------------')
            ...
            if(time_component<=time_to_compare1 and time_component>=time_to_compare2):
                ...
                if(timestamp in DF5.index):
                    timesprev=timestamp-300
                    timesprev2=timestamp-600
                    ...
                    if not self.openPnl.empty:
                        for index,row in self.openPnl.iterrows():
                            sym = row["Symbol"]
                            sym = sym[len(sym)-2:]
                            if sym=='CE'  and DF5['h'][timestamp]>=row['Slfixed'] :
                    ...
                    if (putc<3 and callc<3 and backTestTools.getCurrentExpiry(timestamp))!=backTestTools.getCurrentExpiry(timestamp+86400) and DF5['RSI'][timesprev2]<=60  and Trade==True  and DF5['RSI'][timesprev]>60:
                        ...
                        try:
                            ...
                            sl=min(DF5['l'][timestamp-300],DF5['l'][timestamp-600])
                        ...
                        try:
                            ...
                            sl=max(DF5['h'][timestamp-300],DF5['h'][timestamp-600])
...
if __name__ == "__main__":
    ...
    endDate = datetime.date(2021, 8, 30)
